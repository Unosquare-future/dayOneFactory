...
// Original, fictional styling service — not a real brand.

const ARCHETYPES = [
  { id: 'rustic-rebel', name: 'Rustic Rebel', tagline: 'Weather-worn denim, leather that’s earned it, a quiet nonconformity.', palette: 'ph-7', pairs: ['raw-edge selvedge', 'oiled work boots', 'vintage flannel'], mood: 'ph-7' },
  { id: 'modern-muse', name: 'Modern Muse', tagline: 'Sharp silhouettes, soft fabrics, an instinct for the one perfect piece.', palette: 'ph-10', pairs: ['tailored wool trouser', 'silk crewneck', 'sculptural heel'], mood: 'ph-10' },
  { id: 'adventure-sport', name: 'Adventure Sport', tagline: 'Built to move. Fabrics that breathe. Nothing precious about any of it.', palette: 'ph-9', pairs: ['technical shell', 'merino base', 'trail-ready crossbody'], mood: 'ph-9' },
  { id: 'coastal-sport', name: 'Coastal Sport', tagline: 'Salt air, long afternoons, linen that gets better with every wash.', palette: 'ph-4', pairs: ['washed linen shirt', 'drawstring pant', 'suede loafer'], mood: 'ph-4' },
  { id: 'modern-prep', name: 'Modern Prep', tagline: 'Collegiate, but grown up. Cashmere with a little bit of swagger.', palette: 'ph-11', pairs: ['cable-knit cashmere', 'chino trouser', 'leather penny loafer'], mood: 'ph-11' },
  { id: 'boho-dreamer', name: 'Boho Dreamer', tagline: 'Soft layers, found objects, a wardrobe that tells stories.', palette: 'ph-8', pairs: ['hand-block cotton', 'vintage denim', 'beaded leather sandal'], mood: 'ph-8' },
  { id: 'quiet-classic', name: 'Quiet Classic', tagline: 'Nothing shouts. Everything fits. Five pieces that work for ten years.', palette: 'ph-5', pairs: ['white oxford', 'straight-leg denim', 'unlined trench'], mood: 'ph-5' },
  { id: 'studio-minimal', name: 'Studio Minimal', tagline: 'Architectural lines, one neutral, and a single thoughtful detail.', palette: 'ph-2', pairs: ['boxy tee', 'pleated wide-leg', 'leather derby'], mood: 'ph-2' },
];

const VARIANTS = [
  {
    id: 'swipe', name: 'Style Shuffle',
    kind: 'Swipe',
    thumb: 'ph-3',
...
const VISITORS = [
  {
    id: 'maya', name: 'Maya', age: 23,
    channel: 'Paid social — Instagram',
    device: 'Mobile', session: 'First-time',
  ...
  {
    id: 'robert', name: 'Robert', age: 54,
    channel: 'Organic search — Google',
    device: 'Desktop', session: 'First-time',
  ...
  {
    id: 'priya', name: 'Priya', age: 38,
    channel: 'Email — winback',
    device: 'Mobile', session: 'Lapsed client (11 mo)',
  ...
  {
    id: 'jordan', name: 'Jordan', age: 29,
    channel: 'Referral — friend link',
    device: 'Mobile', session: 'First-time',
    signals: ['arrived with friend’s archetype', 'browsed 4 product pages before signup', 'fashion-forward query patterns'],
    recommend: 'builder',
    confidence: 79,
    reasoning: [
...
const RESULTS_BY_VARIANT = VARIANTS.map(v => ({
  id: v.id, name: v.name,
...
const SWIPE_CARDS = [
  ...
  { id: 11, label: 'Sage utility jacket, straight jean, white sneaker', ph: 'ph-4', tags: ['casual','fresh'] },
...
const TOT_PAIRS = [
  ...
  { q: 'Color that follows you home.', a: { ph:'ph-4', label:'sage + cream' }, b: { ph:'ph-12', label:'camel + ink' } },
...
const CLASSIC_QS = [
  { q: 'Which best describes how you want to feel in your clothes?', opts: ['Pulled together and sharp', 'Soft and at ease', 'Sporty and ready', 'A little unexpected'] },
  { q: 'Where are you wearing most of your wardrobe?', opts: ['Work and client meetings', 'Studio, freelance, coffee shops', 'Kids, errands, real life', 'Travel and long weekends'] },
  { q: 'Pick a neutral you’d build around.', opts: ['Cream', 'Charcoal', 'Camel', 'Soft navy'] },
  { q: 'What are you missing right now?', opts: ['A great jacket', 'One perfect knit', 'Pants that actually fit', 'Something that feels like me'] },
  { q: 'Price you’re happiest paying for a staple item.', opts: ['Under $75', '$75–$150', '$150–$300', '$300+'] },
];
...
const BUILDER_ITEMS = [
  ...
  { id:'sho-4', cat:'Shoes', label:'Suede loafer — sage', ph:'ph-4' },
  { id:'acc-1', cat:'Layer', label:'Unlined trench — oat', ph:'ph-5' },
  { id:'acc-2', cat:'Layer', label:'Wool overcoat — camel', ph:'ph-11' },
  { id:'acc-3', cat:'Layer', label:'Shell jacket — sage', ph:'ph-9' },
  { id:'acc-4', cat:'Layer', label:'Suede bomber — terracotta', ph:'ph-8' },
];

// Pick archetype helpers (deterministic-ish)
function archetypeFor(flowId, payload) {
  // Simple heuristic per variant so the reveal feels earned.
  if (flowId === 'swipe') {
...
Object.assign(window, {
  ARCHETYPES, VARIANTS, VISITORS, ROUTING_RULES, RESULTS_BY_VARIANT,
  SWIPE_CARDS, TOT_PAIRS, MOOD_BOARDS, CLASSIC_QS, BUILDER_ITEMS, archetypeFor,
});
